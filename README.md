...
### 3. Установка системных утилит (Gentoo/Arch)
...
sudo emerge sys-apps/ripgrep-all  # Gentoo
sudo pacman -S ripgrep-all          # Arch

# Визуализация дерева директорий (опционально)
...
```bash
python main.py
# Если ваш файл называется assistens.py, то:
# python assistens.py
```
...
Параметры задаются в `config.py`. Можно переопределить через переменные окружения (env).
